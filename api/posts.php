    ...
    if ($method == 'GET') {
        ...
        if ($_POST != null && !$id) {
            if (count($post)) {
                ...
                $author = $_POST['author'];
                DB::query("INSERT INTO $tableName VALUES(null, :title, :content, :author, null, null)", array(':title' => $title,
':content' => $content, ':author' => $author, ));
        ...
        if ($post != null) {
            ...
            $author = $_PUT['author'];
            DB::query("UPDATE $tableName SET title=:title, content=:content, author=:author WHERE id=:id", array(':title' => $title,
